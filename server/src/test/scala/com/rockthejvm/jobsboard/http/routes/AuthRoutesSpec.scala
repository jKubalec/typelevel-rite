  ...
  "AuthRoutes" - {
      ...
      for {
        ...
        response <- routes.orNotFound.run(
          Request(method = Method.POST, uri = uri"/auth/reset")
            .withEntity(ForgotPasswordInfo(honzaEmail))
        )
        userMap <- userMapRef.get
      } yield {
        //  assertions here
        response.status shouldBe Status.Ok
        userMap should contain key (honzaEmail)
      }
    }

    "should return a 200 - Ok when recovering a password for a valid user/token combination" in {
      for {
        userMapRef <- Ref.of[IO, Map[String, String]](Map(honzaEmail -> "abcd1234"))
        auth       <- IO(probedAuth(Some(userMapRef)))
        routes     <- IO(AuthRoutes(auth, mockedAuthenticator).routes)
        response <- routes.orNotFound.run(
          Request(method = Method.POST, uri = uri"/auth/recover")
            .withEntity(RecoverPasswordInfo(honzaEmail, "abcd1234", "scala-rockstar"))
        )
        userMap <- userMapRef.get
      } yield {
        //  assertions here
    ...
    "should return a 403 - Forbidden when recovering a password for a user with an incorrect token " in {
      for {
        ...
        response <- routes.orNotFound.run(
          Request(method = Method.POST, uri = uri"/auth/recover")
            .withEntity(RecoverPasswordInfo(honzaEmail, "wrong-token", "scala-rockstar"))
        )
        userMap <- userMapRef.get
      } yield {
        //  assertions here
